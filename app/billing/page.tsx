...
import { isPersonalMode } from '@/lib/app-mode';
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 pb-24 lg:pb-6 space-y-6">
        <section className="bg-card border border-border rounded-xl p-6 space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Appka działa w trybie personal (tylko dla Ciebie) — bez limitów planu, płatności i rejestracji nowych kont.
          </p>
          <p className="text-sm text-muted-foreground">
            Gdy zaczniesz udostępniać ją innym (np. po założeniu JDG), przełącz zmienną środowiskową{' '}
            <code className="rounded bg-secondary px-1.5 py-0.5 text-xs">APP_MODE=commercial</code> — plany, limity i
            Stripe wrócą do działania bez zmian w kodzie.
          </p>
        </section>
      </main>
    );
