...
### Sprint 11.2 — komentarze (Instagram + Facebook), zaimplementowane (2026-09-14)

**[PO]:** TASK-11.2.1 wcześniej blokujące — teraz odblokowane. Właściciel samodzielnie zweryfikował na własnych kontach deweloperskich: (1) w TikTok Developer Portal lista dostępnych scope'ów appki nie zawiera niczego związanego z komentarzami — TASK-11.2.2 potwierdzone, TikTok wykluczony z tej dostawy; (2) w oficjalnej dokumentacji Meta "Instagram (IG) Comment" API — mechanika read/reply/hide/delete w pełni udokumentowana i wykonalna; (3) na własnym Meta App Dashboard, w kolejce "App Review submissions → New requests" — `pages_manage_engagement` (Facebook) i `instagram_manage_comments` (Instagram) już tam figurują, złożone wcześniej niezależnie od tej sesji. Decyzja PO: zbudować i przetestować kod NA WŁASNYM koncie właściciela zanim złoży formalny wniosek App Review — konta Admin/Developer/Tester na appce Meta mogą korzystać z niezatwierdzonych jeszcze uprawnień w trybie deweloperskim, dokładnie ten sam wzorzec co piaskownica TikToka (target users/testerzy) już używana w tym projekcie do publikacji. DM (Messenger/IG DM) świadomie POZA zakresem — wymaga osobnych uprawnień (`instagram_manage_messages`/`pages_messaging`), niezweryfikowanych przez właściciela w tej sesji; budowanie pod nie byłoby budowaniem pod hipotezę, dokładnie to czego ten projekt unika od TASK-1.5.3/1.5.4. YouTube nadal odłożone (TASK-11.2.3 flagowało ryzyko kosztownej weryfikacji bezpieczeństwa CASA u Google dla `youtube.force-ssl` — nie zbadane w tej sesji, nie zakładać wykonalności).

**[Architekt]:** `SocialComment` powiązany z `PublishJob` (przez `remotePostId`), nie z `SocialAccount` bezpośrednio — appka reaguje tylko na komentarze pod WŁASNYMI publikacjami, nie enumeruje całej historii posta na koncie (mniejszy zakres uprawnień w praktyce, nawet jeśli scope teoretycznie pozwala na więcej). Wykrywanie: polling w istniejącym dziennym cronie, nie webhook — webhook Meta wymagałby osobnego App Review + weryfikacji publicznego URL callback, kolejnej decyzji biznesowej poza TASK-11.2.1, którą PO nie podjął w tej sesji. Dedupe przez unikalny klucz `(publishJobId, externalCommentId)` w bazie, nie cooldown czasowy jak reszta powiadomień — komentarz to zdarzenie dyskretne (albo jest nowy, albo nie), nie okresowy nudge. Reply suggestion: honest fallback — brak sugestii (Claude nieskonfigurowany albo sam oceni że nie da się sensownie odpowiedzieć) po prostu nie pokazuje przycisku "Wyślij", zamiast wymuszać szablon, którego nie da się uczciwie uogólnić na odpowiedź na czyjś komentarz (inaczej niż caption/coaching, gdzie szablon zapasowy ma sens).

**[Inżynier]:** `lib/server/social-oauth.ts` — `pages_manage_engagement` dopisane do domyślnego scope Facebooka, `instagram_manage_comments` do Instagrama (konto połączone wcześniej wymaga ponownego połączenia, żeby dostać token z nowym scope). Migracja `20260914073007_social_comments` (model `SocialComment` + `CommentStatus`, `User.telegramReplyingToCommentId`). `lib/server/social-comments.ts` — `fetchNewComments`/`postReply` (Instagram: `replies` edge; Facebook: nested `comments` edge — obie mechaniki z oficjalnej dokumentacji Meta, nie zgadywane), `suggestReply` (Claude, `callClaudeTool`, ten sam mechanizm co reszta appki), `detectAndNotifyNewComments` (ósma funkcja w dziennym cronie `telegram-digest`, zero nowego slotu), `acceptSuggestedReply`/`sendCustomReply`/`ignoreComment`. Telegram webhook: przyciski `commentreply`/`commentcustom`/`commentignore` (`handleCommentCallback`), sesja `telegramReplyingToCommentId` dla "Napisz własną" (ten sam wzorzec co edycja/harmonogram, slash command zawsze wygrywa jako escape hatch).

**[QA]:** `tests/api/social-comments.test.ts` (nowy, 12 testów) — wykrywanie nowej Instagram/Facebook komentarza z sugestią Claude, dedupe już zapisanego komentarza, honest brak sugestii bez przycisku "Wyślij", **test wstrzyknięcia promptu** (TASK-11.2.7 — treść komentarza z próbą "Ignore previous instructions..." trafia WYŁĄCZNIE do pola danych JSON, nigdy do system promptu), błąd tokenu na jednym jobie nie blokuje drugiego, accept/custom/ignore + bramka własności (obcy user nie może działać na cudzym komentarzu) + błąd platformy zostawia komentarz PENDING zamiast fałszywie REPLIED. `tests/api/telegram-comment-actions.test.ts` (nowy, 5 testów) — pełny webhook: przycisk Wyślij/Ignoruj/Napisz własną, slash command jako escape hatch w trakcie sesji "Napisz własną", bramka własności na poziomie callbacku. `tests/api/cron-telegram-digest.test.ts` (+1 asercja: `commentsDetected: 0` bez tokenu testowego). Pełna suita: **376/376 w obu trybach APP_MODE**, tsc czyste (poza dwoma znanymi błędami `.mjs`), build czysty.

Status: [x] zaimplementowane (komentarze Instagram+Facebook w całości; DM/TikTok/YouTube świadomie poza zakresem) → [x] testy napisane i zielone → [x] zweryfikowane (tsc, build czyste) → [x] zamknięte (PR #84 zmergowany, wdrożenie produkcyjne potwierdzone przez postfly.pl/api/health) → [ ] weryfikacja żywych danych (wymaga ponownego połączenia kont z nowym scope i realnego komentarza na koncie właściciela) → [ ] formalne zgłoszenie App Review u Meta (świadomie odłożone do czasu działającego demo, decyzja PO)
...
Status: [x] zaimplementowane (TASK-9.2/9.3/9.5 zamknięte; TASK-9.1/9.4 świadomie odłożone z jasnym powodem) → [x] testy napisane i zielone (499/499) → [x] zweryfikowane (tsc, build czyste) → [x] zmergowane (PR #96) → [x] wdrożone (auto-deploy przez integrację Vercel-GitHub na merge do `main`, endpointy 2FA i strony account/login/terms zweryfikowane na produkcji przez HTTP)

---

### EPIC 10 — redesign UX/UI panelu, Faza 1 (2026-09-15)

Użytkownik poprosił o wykonanie EPIC 10 zaraz po zamknięciu EPIC 9, mimo wcześniejszej rekomendacji agenta-prezesa, żeby wstrzymać budowę nowych funkcji na rzecz walidacji produktu - świadoma decyzja właściciela, wykonana zgodnie z instrukcją.

**[Architekt]:** Kluczowe odkrycie na starcie: `UX_AUDIT.md`, do którego odwołuje się sekcja 0.1 tego dokumentu ("Analizuje istniejący `UX_AUDIT.md`..."), **nie istnieje** - usunięty 2026-09-12 przy konsolidacji dokumentacji (commit `146dc93`, `-268` linii), jego ustalenia miały zostać wchłonięte przez ten dokument, ale w praktyce nie przetrwały w żadnej szczegółowej formie. TASK-10.1 to więc nowy audyt, nie rozszerzenie starego - odtworzony przez statyczny przegląd kodu (Explore-agent po wszystkich 18 stronach `app/**/page.tsx`), jawnie zastrzeżony jako NIE oparty na realnych zrzutach ekranu jak oryginał (brak narzędzia do automatyzacji przeglądarki w tym środowisku). Drugie odkrycie: tryb ciemny domyślny (część zasady UI Designera z sekcji 0.1) **już istniał** (`defaultTheme="dark"` w `app/layout.tsx`) - nic do zrobienia, tylko potwierdzenie.

**[UX Researcher → UI Designer → Design Critic, w tej kolejności zgodnie z ustalonym workflow sekcji 0.1]:** Ranking gęstości decyzji: najgorszy `/schedule` (~16+ niepowiązanych decyzji, 1187 linii - główny flow planowania/publikacji appki), potem `CommunityPanel` (~7), `Dashboard` (~6-7), `app/account/page.tsx` (5 płaskich, zawsze rozwiniętych sekcji - Telegram/profil/autopilot/2FA/usuń konto, samoistnie pogorszone w tej sesji przez kolejne dokładanie funkcji do tej samej strony). Świadoma decyzja o zakresie: **`/schedule` NIE zostało przebudowane w tej turze** - to największy, najbardziej biznesowo-krytyczny ekran appki, a przebudowa "na ślepo" bez możliwości wizualnej weryfikacji w tym środowisku, w jednej turze, bez iteracji z właścicielem, byłaby dokładnie tym ryzykiem, przed którym ostrzega własna zasada tego projektu ("nigdy nie buduj pod hipotezę / ryzykownie bez weryfikacji"). Naprawione zamiast tego: `app/account/page.tsx` (akordeon, `components/CollapsibleSection.tsx`, framer-motion, `useReducedMotion` respektowane, ten sam profil animacji co `components/AppShell.tsx`) i `components/CommunityPanel.tsx` (formularze "Dodaj sprzedaż"/"Dodaj fana" schowane za przyciskiem, zwijają się po sukcesie).

**Design Critic, realne znalezisko (nie subiektywne "mi się nie podoba")**: policzony (nie oszacowany) błąd kontrastu WCAG AA - `bg-destructive text-destructive-foreground` (przyciski "Usuń konto trwale", "Wyłącz 2FA") dawało **~2.9:1 w trybie ciemnym** (domyślnym!) i **~4.3:1 w jasnym**, oba poniżej wymaganych 4.5:1 dla zwykłego tekstu. Naprawione przez przyciemnienie `--destructive` w `:root` i `--destructive-foreground` w `.dark` (`styles/theme.css`) - ten sam wzorzec co już istniejące `--primary-foreground` (ciemny tekst na jasnym, nasyconym tle). Zabezpieczone testem `tests/unit/theme-contrast.test.ts`, który parsuje PRAWDZIWY plik CSS (nie kopię wartości) i liczy realny kontrast WCAG dla 9 par tekst/tło w obu motywach (18 asercji) - żaden przyszły edyt tokenu nie przejdzie CI, jeśli znowu zepsuje kontrast.

**[Inżynier]:** `lib/feature-flags.ts` (`isFeatureEnabled`, `NEXT_PUBLIC_FEATURE_FLAGS`) - domyka też TASK-6.5. Świadomie NIE zastosowane do zmian z tej tury (ściśle addytywne, appka ma jednego właściciela w `personal` - realna populacja do stopniowego wdrożenia nie istnieje jeszcze) - czeka na przebudowę `/schedule`, gdzie skala ryzyka faktycznie uzasadnia stopniowe włączanie.
