...
Status: [x] zaimplementowane (komentarze Instagram+Facebook w całości; DM/TikTok/YouTube świadomie poza zakresem) → [x] testy napisane i zielone → [x] zweryfikowane (tsc, build czyste) → [ ] zamknięte (PR w przygotowaniu) → [ ] weryfikacja żywych danych (wymaga ponownego połączenia kont z nowym scope i realnego komentarza na koncie właściciela) → [ ] formalne zgłoszenie App Review u Meta (świadomie odłożone do czasu działającego demo, decyzja PO)
