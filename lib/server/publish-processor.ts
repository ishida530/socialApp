...
async function failOrScheduleRetry(
  ...
  const retryMessage = buildRetryMessage(currentAttempt, reason);
...
async function publishToTikTokPhoto(job: PublishInputJob, accessToken: string): Promise<PublishTransportResult> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`TikTok photo publish init failed: ${errorBody || response.statusText}`);
  }
...
async function publishToFacebook(job: PublishInputJob, accessToken: string): Promise<PublishTransportResult> {
  ...
  const params = new URLSearchParams({
    ...
    file_url: sourceUrl,
    title: (job.title?.trim() || job.video.title).slice(0, 255),
    description: composeCaption(job.caption, job.hashtags).slice(0, 5000),
    published: 'true',
  });
...
async function publishToFacebookPhoto(job: PublishInputJob, accessToken: string): Promise<PublishTransportResult> {
  ...
  const params = new URLSearchParams({
    access_token: accessToken,
    url: sourceUrl,
    caption: composeCaption(job.caption, job.hashtags).slice(0, 5000),
    published: 'true',
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(`Facebook photo publish failed: ${errorBody || response.statusText}`);
  }
...
async function publishToInstagram(job: PublishInputJob, accessToken: string): Promise<PublishTransportResult> {
...
async function publishToInstagramPhoto(job: PublishInputJob, accessToken: string): Promise<PublishTransportResult> {
  const igUserId = job.socialAccount.externalId;
  if (!igUserId) {
    throw new Error('Brak externalId konta Instagram Business dla konta social');
  }
...
async function publishToPlatform(job: PublishInputJob, accessToken: string): Promise<PublishTransportResult> {
  if (job.video.mediaType === 'IMAGE') {
    ...
    if (job.socialAccount.platform === 'FACEBOOK') {
      return publishToFacebookPhoto(job, accessToken);
    }
...
async function processClaimedJob(jobId: string) {
  ...
  const tiktokTrackingState =
    job.socialAccount.platform === 'TIKTOK'
      ...
      : null;
  const tiktokSettings: TikTokPublishSettings | undefined =
